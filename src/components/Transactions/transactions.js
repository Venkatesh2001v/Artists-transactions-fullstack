...
class Transactions extends Component {
    ...
    submittingForm = async (event) => {
        ...
        const { type, date, description, amount } = this.state;
        const changeAmount = parseFloat(amount);
        const data = { type, date, description, amount: changeAmount };
        
        if (!type || !amount || !date) {
            this.setState({ isError: true, ErrorMsg: 'All fields are required' });
            return;
        }

        const url = 'https://artists-transactions-backend.onrender.com/transactions';
        const options = {
            method: 'POST',
            headers: {
        ...
        try {
            ...
            if (response.ok) {
                ...
                this.setState({ isError: true, ErrorMsg: 'Failed to add transaction' });
            }
        } catch (error) {
            this.setState({ ErrorMsg: `Error: ${error.message}`, isError: true });
        }
    };
    ...
    render() {
        const { amount, description, date, isError, ErrorMsg } = this.state;
        return (
            <div className="transaction-container">
                ...
                <form onSubmit={this.submittingForm} className="form">
                    ...
                    <div className="input-div">
                        ...
                        <input type="text" className="input-el" onChange={this.onChangeAmount} id='amount' value={amount} />
                    </div>
                    <div className="input-div">
                        <label htmlFor="description">Description:</label>
                        <textarea className="area-el" onChange={this.onChangeDescription} id='description' value={description}></textarea>
                    </div>
                    <div className="input-div">
